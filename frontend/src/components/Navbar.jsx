...
export default function Navbar() {
  ...
  return (
    <nav className={`navbar ${scrolled ? "scrolled" : ""}`.trim()}>
      ...
      <ul className={`navbar-menu ${menuOpen ? "active" : ""}`.trim()}>
        <li>
          <Link
            to="/home"
        ...
        <li className="navbar-menu-controls">
          <div className="language-switch" aria-label={t("nav.language_switcher") || undefined}>
            <button
          ...
          {user ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
              <Link to="/me" onClick={() => setMenuOpen(false)} className="login-btn">
                {t('profile.dropdown.my_profile')}
              </Link>
              <Link to="/me/products" onClick={() => setMenuOpen(false)} className="login-btn">
                {t('profile.dropdown.products')}
              </Link>
              <Link to="/me/settings" onClick={() => setMenuOpen(false)} className="login-btn">
        ...
        {user ? (
          <div className="user-menu" ref={dropdownRef}>
            <button
              ...
              aria-label={t("profile.dropdown.aria_label") || undefined}
            >
              ...
              <div className="user-menu__dropdown">
                ...
                <button className="user-menu__item user-menu__item--danger" onClick={handleLogout}>
                  <svg width="14" height="14" viewBox="0 0 15 15" fill="none">
                    <path d="M5 13H3a1 1 0 01-1-1V3a1 1 0 011-1h2M10 10l3-2.5L10 5M13 7.5H6" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  {t("profile.dropdown.logout")}
                </button>
              </div>
