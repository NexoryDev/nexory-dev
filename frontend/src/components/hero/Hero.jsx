...
function buildSvgLine({ cubeCenter, ep, color, gradId, pathLen }) {
  ...
  grad.setAttribute("x1", cubeCenter.x);
  grad.setAttribute("y1", cubeCenter.y);
  ...
  glow.setAttribute("x1", cubeCenter.x);
  glow.setAttribute("y1", cubeCenter.y);

  glow.setAttribute("x2", ep.x);
  glow.setAttribute("y2", ep.y);
  ...
  line.setAttribute("x1", cubeCenter.x);
  line.setAttribute("y1", cubeCenter.y);
  ...
  const drawLines = useCallback(() => {
    ...
    CARD_CFGS.forEach(({ id, color }, i) => {
      ...
      const pathLen = Math.hypot(
        cardEdge.x - cubeEdge.x,
        cardEdge.y - cubeEdge.y,
      );

      const relCube = {
        x: cubeEdge.x - sceneRect.left,
        y: cubeEdge.y - sceneRect.top,
      };

      const relCard = {
        x: cardEdge.x - sceneRect.left,
        y: cardEdge.y - sceneRect.top,
      };
